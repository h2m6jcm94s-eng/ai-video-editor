...
def _video_encode_args(config: RenderConfig) -> List[str]:
    ...
    if config.video_codec in ("h264_nvenc", "hevc_nvenc"):
        ...
        cq = config.video_crf if config.video_crf and config.video_crf > 0 else COMPILER.NVENC_DEFAULT_CQ
...
# Presets valid for the libx264 intermediate segment encoder.  Some encoder
# presets (e.g. NVENC "p4") are not valid here, so we fall back to "medium".
_LIBX264_PRESETS = frozenset(
    {"ultrafast", "superfast", "veryfast", "faster", "fast", "medium", "slow", "slower", "veryslow", "placebo"}
)


def _segment_video_preset(config: RenderConfig) -> str:
    """Return a libx264-compatible preset for intermediate segment encodes."""
    if config.video_preset in _LIBX264_PRESETS:
        return config.video_preset
    return "medium"
...
def _extract_segment(args) -> Optional[dict]:
    ...
    # Intermediate segment encode: always use libx264 with the configured
    # preset/CRF so the final concat is not built from ultrafast placeholder
    # quality. Pix fmt is forced to yuv420p for broad compatibility.
    cmd = [
        "ffmpeg", "-y",
        "-ss", str(start),
        "-t", str(duration),
        ...
        "-vf", vf,
        "-c:v", "libx264",
        "-preset", _segment_video_preset(config),
