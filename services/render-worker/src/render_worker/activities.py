...
async def fetch_project(project_id: str) -> dict:
    ...
    url = f"{settings.api_base}/internal/projects/{project_id}"
    resp = httpx.get(url, headers=_internal_headers(), timeout=30)
...
async def compile_render(
    ...
    output_path = os.path.join(
        tempfile.gettempdir(),
        f"ave_render_{cutlist_obj.globals.total_duration_s:.0f}s_{activity.info().workflow_run_id}.mp4",
    )
