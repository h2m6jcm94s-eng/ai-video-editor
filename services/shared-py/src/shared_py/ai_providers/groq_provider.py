...
class GroqProvider(AIProvider):
    """Groq Llama 3.3 70B / Mixtral via OpenAI-compatible API. ~800 tok/sec."""
    ...
    def __init__(self, api_key: str = None, model: str = "llama-3.3-70b-versatile"):
        ...
        if not resolved_key:
            raise ValueError("GROQ_API_KEY is not set — set it or remove 'groq' from AI_PROVIDER")
        self.client = OpenAI(
    ...
    def generate_cutlist(self, context: str, schema: Dict[str, Any], max_tokens: int = 4096) -> CutList:
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    raise RuntimeError(f"Groq cut-list generation failed after 3 attempts: {e}")
