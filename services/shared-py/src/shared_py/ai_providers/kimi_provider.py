...
class KimiProvider(AIProvider):
    """Kimi k1.5 via Moonshot OpenAI-compatible API. 2M context window."""

    name = "kimi"
    BASE_URL = "https://api.moonshot.cn/v1"

    def __init__(self, api_key: str = None, model: str = "kimi-k1.5"):
        if OpenAI is None:
    ...
    def _strip_markdown(text: str) -> str:
        """Moonshot models occasionally wrap JSON in markdown code blocks."""
        return (
            text.strip()
            .removeprefix("```json")
            .removeprefix("```")
    ...
    def generate_cutlist(self, context: str, schema: Dict[str, Any], max_tokens: int = 4096) -> CutList:
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    raise RuntimeError(f"Kimi cut-list generation failed after 3 attempts: {e}")

        raise RuntimeError("Kimi returned invalid JSON")
